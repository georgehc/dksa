...
survival_estimator_display_name = 'Cox-Time'
    ...
    for dataset in datasets:
        ...
        if not os.path.isfile(output_train_metrics_filename) or \
            ...
            for hyperparam in hyperparams:
                ...
                for fold_idx, (train_idx, val_idx) \
                    ...
                    net = MLPVanillaCoxTime(fold_X_train_std.shape[1],
                    ...
                    fold_y_train_std = labtrans.fit_transform(*fold_y_train.T)
                    fold_y_val_std = labtrans.transform(*fold_y_val.T)
                    ...
                    if not os.path.isfile(model_filename):
                        if use_early_stopping and not use_cross_val:
                            surv_model.fit(fold_X_train_std, fold_y_train_std,
                                           batch_size, n_epochs,
        ...
        for hyperparam in best_hyperparams:
            ...
            if not use_cross_val:
                ...
                copyfile(val_model_filename[:-3] + '_blh.pickle',
                         model_filename[:-3] + '_blh.pickle')
            ...
            bootstrap_dir = \
                os.path.join(output_dir, 'bootstrap',
                             '%s_%s_exp%d_%s_bs%d_nep%d_nla%d_nno%d_lr%f_test'
                             % (survival_estimator_name, dataset,
                                experiment_idx, val_string, batch_size,
                                n_epochs, n_layers, n_nodes, lr))
