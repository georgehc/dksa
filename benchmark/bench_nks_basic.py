...
from neural_kernel_survival import NKS, NKSDiscrete, Scaler
from survival_datasets import load_dataset


survival_estimator_name = 'nks_basic'
survival_estimator_display_name = 'NKS-Basic'
...
for experiment_idx in range(n_experiment_repeats):
    for dataset in datasets:
        ...
        if not os.path.isfile(output_train_metrics_filename) or \
            ...
            for hyperparam in hyperparams:
                ...
                for cross_val_idx, (train_idx, val_idx) \
                    ...
                    net = nn.Sequential(Scaler(fold_X_train_std.shape[1]))
                    ...
                    model_filename = \
                        os.path.join(output_dir, 'models',
                                     ...
                                     % (survival_estimator_name, dataset,
                                        experiment_idx, batch_size, n_epochs, lr,
        ...
        for hyperparam in best_hyperparams:
            ...
            optimizer = tt.optim.Adam(lr=lr)
            net = nn.Sequential(Scaler(X_train_std.shape[1]))
            ...
            model_filename = \
                os.path.join(output_dir, 'models',
                             '%s_%s_exp%d_bs%d_nep%d_lr%f_nd%d_test.pt'
                             % (survival_estimator_name, dataset,
                                experiment_idx, batch_size, n_epochs, lr,
                                num_durations))
            time_elapsed_filename = model_filename[:-3] + '_time.txt'
            ...
            bootstrap_dir = \
                os.path.join(output_dir, 'bootstrap',
                             '%s_%s_exp%d_bs%d_nep%d_lr%f_nd%d_test'
                             % (survival_estimator_name, dataset,
                                experiment_idx, batch_size, n_epochs, lr,
                                num_durations))
